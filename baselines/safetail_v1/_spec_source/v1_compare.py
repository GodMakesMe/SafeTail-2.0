...
def get_agent_latency(agent,state,task,beta):
  ...
  if task == "yolo":
    state_flattened = np.array(state_flattened).reshape(1,10)
  if task == "instance":
  ...
  # print(state_flattened)
  # print("action_values: ",action_vals[0])
  print("action_values:", np.argmax(action_vals[0]))
  with open('instance_out.txt', 'a') as f:
    print("action_values:", np.max(action_vals[0]), file=f)
...
def get_min_load_latency(state,task , num_nodes , num_sample):

  sampled_nodes = random.sample(range(len(state["LOAD"])), num_sample)
  # sort index based on propogation delay
  sorted_nodes = sorted(sampled_nodes, key=lambda k: state['LOAD'][k])
...
def get_min_propogation_latency(state,task,num_nodes,num_sample):
  # randomly select num_sample nodes
  sampled_nodes = random.sample(range(len(state["LOAD"])), num_sample)
  # sort index based on propogation delay
  sorted_nodes = sorted(sampled_nodes, key=lambda k: state['PROPOGATION'][k])
  selected_nodes = sorted_nodes[:num_nodes]
  obs_latency = 100000000
  for node in selected_nodes :
    computaion_delay_node = get_computation_delay(state,node,task)
...
def get_random_file_size(task ):
    ...
    with open('noise/noise_final_data_19May.csv', 'rb') as f:
        noise_df = pd.read_csv(f)
...
def main():
    ...
    if not os.path.exists(args.run_config):
        ...
        if loaded_model:
            ...
            # min_load_latencies = []
            min_load_latencies_1 = []
            min_load_latencies_2 = []
            min_load_latencies_3 = []
            min_propagation_latencies = []
            min_propagation_latencies_2 = []
            min_propagation_latencies_3 = []
            ...
            for run in range(5):
              for index in df[0:1420].index:
                  ...
                  for edge_device_no in range(beta):
                      current_load.append(df.iloc[ index , 6+ edge_device_no])
                  ...
                  # min_load_latency = get_min_load_latency(state,task,1,3)
                  min_load_latency_1 = get_min_load_latency(state, task, 1,3)
                  min_load_latency_2 = get_min_load_latency(state, task, 2,3)
                  min_load_latency_3 = get_min_load_latency(state, task, 3,3)
                  ...
                  min_propogation_delay = get_min_propogation_latency(state,task,1,3)
                  min_propogation_delay_2 = get_min_propogation_latency(state,task,2,3)
                  min_propogation_delay_3 = get_min_propogation_latency(state,task,3,3)
                  ...
                  min_load_latencies_1.append(min_load_latency_1)
                  min_load_latencies_2.append(min_load_latency_2)
                  min_load_latencies_3.append(min_load_latency_3)
                  ...
                  min_propagation_latencies_2.append(min_propogation_delay_2)
                  min_propagation_latencies_3.append(min_propogation_delay_3)
                  access_rate.append(gamma)
            

           
      
            data = {'MIN': global_min_delay_latencies, 'DQN': agent_latencies, 'RAND_1': random_latencies_1, 'RAND_2': random_latencies_2 , 'RAND_3': random_latencies_3 , 'RAND_4': random_latencies_4, 'MIN_LOAD_1': min_load_latencies_1,'MIN_LOAD_2': min_load_latencies_2,'MIN_LOAD_3':min_load_latencies_3, 'MIN_PROP': min_propagation_latencies, 'MIN_PROP_2' :  min_propagation_latencies_2, 'MIN_PROP_3': min_propagation_latencies_3 }
            ...
            for i in [50,80,90,95,99]:
              ...
              min_load_latency_percentile = np.percentile(min_load_latencies_1, i)
              ...
              min.append(min_latency_percentile)
              dqn.append(agent_latency_percentile)
              random_1.append(random_latency_1_percentile)
              random_2.append(random_latency_2_percentile)
              random_3.append(random_latency_3_percentile)
              random_4.append(random_latency_4_percentile)
              min_load.append(min_load_latency_percentile)
              min_prop.append(min_propagation_latency_percentile)
            ...
            # Plotting
            latencies = ["50%ile", "80%iie","90%ile", "95%ile", "99%ile"]

            index = np.arange(len(latencies))

            plt.bar(index, min, width=bar_width, label='min', edgecolor=colors[0], hatch=patterns[0], color='white')
            plt.bar(index + bar_width, dqn, width=bar_width, label='dqn', edgecolor=colors[1], hatch=patterns[1], color='white')
            plt.bar(index + 2*bar_width, random_2, width=bar_width, label='rand2', edgecolor=colors[2], hatch=patterns[2], color='white')
            plt.bar(index + 3*bar_width, min_load, width=bar_width, label='min_load', edgecolor=colors[3], hatch=patterns[3], color='white')
            plt.bar(index + 4*bar_width, min_prop, width=bar_width, label='min_prop', edgecolor=colors[4], hatch=patterns[4], color='white')
            plt.ylabel('Execution Time (s)')
            plt.xticks(index + 1.5*bar_width, latencies)
            # Add legend on the right side
            plt.legend(loc='upper left',ncol=2)
            #Add grid lines
            plt.grid(True, linestyle='--', alpha=0.7)

            plt.savefig(args.run_config + f"/testing_curve.png", bbox_inches='tight')
              ...
              # plt.bar(np.arange(3),[dic['random_2'],dic['min'],dic['min_load'] , dic['min_prop']], width=bar_width,edgecolor = colors[0], hatch=patterns[0], color='white')
